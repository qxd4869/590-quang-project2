...
// handle each object and calculate collision and push back force
const checkCrash = () => {
  // if we have attack
    // get all characters
  ...
  for (let i = 0; i < keys.length; i++) {
    const speedX = characters[keys[i]].speedX;
    const speedY = characters[keys[i]].speedY;
    if (speedX < 0) {
  ...
  for (let j = 0; j < keys.length; j++) {
    ...
    for (let k = j + 1; k < keys.length; k++) {
      ...
      if (hit) {
        const result = crashHelper(keys[j], keys[k]);
